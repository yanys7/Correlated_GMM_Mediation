...

{
    ...
    for ( size_t i = 0; i < q; i++ ) {
      for ( size_t j = i; j < q; j++ ) {
        if ( std::abs( CovM( i, j ) ) >= 1.0 && i != j ) { // not Cov[ i, j ]
  ...
  {
    /** res1 = Y - M * beta_m - beta_a * A - C * beta_c */
    res1 = Y;
    for ( size_t i = 0; i < n; i ++ )
      res1[ i ] -= beta_a[ 0 ] * A[ i ];
   ...
   T PostDistribution( Data<T> beta_m, Data<T> alpha_a, Data<T> beta_a, T sigma_e, T sigma_g, Data<T> beta_c, Data<T> alpha_c )
   {
     T llh1 = 0.0;
     T llh2 = 0.0;
     ...
     for ( size_t i = 0; i < n; i++ ) {
       ...
       for ( size_t j = 0; j < q; j++ ) {
         ...
         T meanc2 = alpha_a[ j ] * A[ i ];
         for ( size_t j2 = 0; j2 < w2; j2++ ) {
           meanc2 += alpha_c( j2, j ) * C2( i, j2 );
         }
         llh2 += log_normal_pdf ( meanc2, std::sqrt( sigma_g ), M(i, j) );
       ...
       for ( size_t j1 = 0; j1 < w1; j1++ ) { meanc1 += C1( i, j1 ) * beta_c[ j1 ]; }
  ...
  if ( ( it % 5 ) != 1 ) {
    ...
    for ( auto cluster : allClusters ) {
     ...
     hmlp::Data<T> sigma(1, n_mixtures, 2.0);
     //mu[ 0 ] = -2.0; mu[ 1 ] = -2.0; mu[ 2 ] = -2.0; mu[ 3 ] = -0.2;
     mu[ 0 ] = -4.6; mu[ 1 ] = -3.0; mu[ 2 ] = -3.0; mu[ 3 ] = -0.1;
     sigma[ 3 ] = 0.5; sigma[ 0 ] = 4.0;
     for ( size_t k = 0; k < n_mixtures; k ++ ) {
     ...
     /** update theta_1 */
     theta_temp = theta_1;
     hmlp::Data<T> mu_t1(1, n_mixtures, 0.0);
     hmlp::Data<T> sigma_t1(1, n_mixtures, 1.0);
     //mu_t1[ 0 ] = 0.5; mu_t1[ 1 ] = 0.5; mu_t1[ 2 ] = 0.5; mu_t1[ 3 ] = 0.5;
     mu_t1[ 0 ] = 0.0; mu_t1[ 1 ] = 0.0; mu_t1[ 2 ] = 0.0; mu_t1[ 3 ] = 0.0;
     sigma_t1[ 0 ] = 1.0;
     ...
     for ( size_t k = 0; k < n_mixtures; k ++ ) {
       ...
       hastings = hastings - ( theta_temp[ k ] - mu_t1[ k ] ) * ( theta_temp[ k ] - mu_t1[ k ] )/2/sigma_t1[ k ] + ( theta_1[ k ] - mu_t1[ k ] ) * ( theta_1[ k ] - mu_t1[ k ] )/2/sigma_t1[ k ];
       my_unif.rand( 0.0, 1.0 );
       if ( hastings > std::log( my_unif[ 0 ] ) ) {
      ...
      if ( it > burnIn && it % 10 == 0 )
      {
        ...
        my_samples( count, 3* (int)q + 9 ) = PostDistribution( beta_m, alpha_a, beta_a, sigma_e, sigma_g, beta_c, alpha_c );
